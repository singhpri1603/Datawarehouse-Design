...
$query_del='DELETE FROM `informative_temp2` WHERE 1';
        $result = mysqli_query($link, $query_del);
$query_first='insert into `informative_temp2` SELECT probe.pb_id,microarray_fact.exp,samplejoined.p_id,probe.UID FROM clinical_fact join disease on clinical_fact.ds_id=disease.ds_id left join (SELECT p_id,s_id FROM clinical_fact where clinical_fact.s_id!="null") as samplejoined on samplejoined.p_id=clinical_fact.p_id left join microarray_fact on microarray_fact.s_id=samplejoined.s_id left join probe on probe.pb_id=microarray_fact.pb_id where disease.name="'.$name.'" and samplejoined.s_id!="NULL" ORDER BY `probe`.`UID` ASC';
$result = mysqli_query($link, $query_first);



$query_del='DELETE FROM `informative_temp3` WHERE 1';
        $result = mysqli_query($link, $query_del);
$query_second='INSERT INTO informative_temp3
SELECT informative_temp2.pb_id, informative_temp2.exp, informative_temp2.p_id, informative_temp2.UID
FROM informative_temp2
RIGHT JOIN informative_genes_temp ON informative_genes_temp.gene_id = informative_temp2.UID';
$result = mysqli_query($link, $query_second);

$query = 'SELECT distinct informative_temp3.p_id FROM informative_temp3';
...
while($row1 = mysqli_fetch_assoc($result))
{
    ...
    $query2 = 'SELECT pb_id,exp,p_id,UID FROM informative_temp3 where p_id="'.$pat_id.'" ORDER BY UID ASC';
...
$new_pat_query='SELECT * FROM `TABLE 33` right join informative_genes_temp on informative_genes_temp.gene_id=`TABLE 33`.test_user_uid ORDER BY `TABLE 33`.`test_user_uid` ASC';
    ...
    $r_of_a=array();
    $table_return="";
    $table_return.="<html><table>";
    $query_del='DELETE FROM ra_temp WHERE 1';
    $result_del = mysqli_query($link, $query_del);
    
    
    $table_return.='<tr><td>TEST1</td><td>TEST2</td><td>TEST3</td><td>TEST4</td><td>TEST5</td></tr>';
    ...
    foreach($container as $key=>$value)
    {
     ...
     $table_return.="<tr><td>".$pca_temp1."</td><td>".$pca_temp2."</td><td>".$pca_temp3."</td><td>".$pca_temp4."</td><td>".$pca_temp5."</td></tr>";
     $query_in="insert into ra_temp values ('".$pca_temp1."','".$pca_temp2."','".$pca_temp3."','".$pca_temp4."','".$pca_temp5."')";
     $result_in = mysqli_query($link, $query_in);
    }
   $table_return.="</table></html>";
...
//    
    ...
    $output=array();
    $output['noofrows']=$counter_n;
    $output['result']=$table_return;
