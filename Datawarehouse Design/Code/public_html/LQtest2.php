...
$output = array();  //output 2D array will store all outputs we want to return to UI webpage
$name1 = "ALL";
$name2 = "AML";
$goid = "7154";
if(isset($_GET["name1"])){
...
while($row1 = mysqli_fetch_assoc($result))
{
    ...
    $pat_id=$row1['p_id'];
    $query2 = 'SELECT exp FROM clinical_fact join disease on clinical_fact.ds_id=disease.ds_id left join (SELECT p_id,s_id FROM clinical_fact where clinical_fact.s_id!="null") as samplejoined on samplejoined.p_id=clinical_fact.p_id left join microarray_fact on microarray_fact.s_id=samplejoined.s_id left join probe on probe.pb_id=microarray_fact.pb_id left join gene_fact on gene_fact.UID=probe.UID where disease.name="'.$name1.'" and gene_fact.go_id="'.$goid.'" AND clinical_fact.p_id="'.$pat_id.'" ORDER BY probe.pb_id ASC';
...
//    echo '</pre>';
/////////////////////
    $query = 'SELECT distinct clinical_fact.p_id FROM clinical_fact join disease on clinical_fact.ds_id=disease.ds_id left join (SELECT p_id,s_id FROM clinical_fact where clinical_fact.s_id!="null") as samplejoined on samplejoined.p_id=clinical_fact.p_id left join microarray_fact on microarray_fact.s_id=samplejoined.s_id left join probe on probe.pb_id=microarray_fact.pb_id left join gene_fact on gene_fact.UID=probe.UID where disease.name="'.$name2.'" and gene_fact.go_id="'.$goid.'"';
...
while($row1 = mysqli_fetch_assoc($result))
{
    ...
    $pat_id=$row1['p_id'];
    $query2 = 'SELECT exp FROM clinical_fact join disease on clinical_fact.ds_id=disease.ds_id left join (SELECT p_id,s_id FROM clinical_fact where clinical_fact.s_id!="null") as samplejoined on samplejoined.p_id=clinical_fact.p_id left join microarray_fact on microarray_fact.s_id=samplejoined.s_id left join probe on probe.pb_id=microarray_fact.pb_id left join gene_fact on gene_fact.UID=probe.UID where disease.name="'.$name2.'" and gene_fact.go_id="'.$goid.'" AND clinical_fact.p_id="'.$pat_id.'" ORDER BY probe.pb_id ASC';
    
    $output["query4"] = $query2;
    $result2 = mysqli_query($link, $query2);
...
//    echo '</pre>';
    //echo 'result';         
    //echo '<pre>'; 
  $output["result1"] = $all_disease_pca_among_each;
//    echo '</pre>';
//    echo 'r';         
...
//    echo '</pre>';
//    echo '<pre>'; 
    $output["result3"] = $all_disease_pca_among_each/$div;
